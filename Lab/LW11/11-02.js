...
let server = http.createServer((req,resp)=>{
    if(url.parse(req.url).pathname === '/client' && req.method === 'GET'){
        ...
        resp.end(fs.readFileSync('./11-02a.html'));
    }
    if(url.parse(req.url).pathname === '/download' && req.method === 'POST'){
...
webserver.on('connection', (ws)=>{
    ws.on('message',(m)=>{
        ...
        let rfile = fs.createReadStream(`./download/${m}`);
        rfile.pipe(duplex);
    })
})